...
- Ultra chỉ đổi thứ tự ưu tiên xếp hàng, không đảm bảo số comment hoặc thời gian hoàn thành. Nếu yêu cầu Ultra hết hạn hoặc bị admin hủy mà chưa có chuỗi nào mở turn đầu cho bài đó, hệ thống hoàn đúng 1 lượt Ultra một lần qua sự kiện ledger `ultra_refunded` (RPC `settle_engagement_boosts`, migration `20260918120000`); replay/đối soát lại không đổi số dư lần hai.
- **Chiến dịch** chỉ dành cho admin: tạo campaign, chọn bài, nhập JSON, chọn vote/comment/reply, quota, cooldown, lịch chạy, pause/cancel và xem tiến độ toàn hệ thống. Ngoài ra admin có **Chiến dịch nhanh** (R4 trong `PLAN_PRODUCT_9_10.md`): năm lựa chọn nhóm user, bài đích, số chuỗi/bài, khung giờ và preset An toàn/Cân bằng; server tự ghép người theo quota/cặp/giờ, preview capacity trước khi chạy và các thông số kỹ thuật chỉ nằm trong mục nâng cao.
