...
| `engagement_campaigns` | Campaign vote/comment + phạm vi bài + quota; campaign sinh từ chiến dịch nhanh ghi thêm `preset` (safe/balanced) |
| `engagement_tasks` | Task hàng đợi (lease, `idempotency_key`, `session_required`) |
| `engagement_events` | Nhật ký claim/succeed/fail/retry (dọn định kỳ) + sự kiện `ultra_refunded` |
...
| `engagement_boost_requests` | Yêu cầu ưu tiên bài do admin Push hoặc user đổi Ultra; migration `20260918120000` thêm `refunded_at`/`refund_reason` và RPC `settle_engagement_boosts` hoàn Ultra đúng một lần khi hết hạn/hủy mà chưa có chuỗi mở turn đầu |
...
| `content_presets` | Preset đăng bài AI (community + terms + body_type); seed sẵn "Cải tiến mỗi ngày" (community 35, terms 178/368/274) — worker không hardcode |
| `content_items` | Bài trong kho AI, chống nhập trùng bằng `content_hash` UNIQUE; trạng thái biên tập draft/review/approved/rejected/archived |
| `content_revisions` | Snapshot bất biến khi admin duyệt (UNIQUE theo item + số revision); lịch đăng chỉ trỏ vào revision, sửa draft không đổi bản đã duyệt |
| `content_revision_approvals` | Phê duyệt của user đích (UNIQUE theo revision + username) — chỉ user được gán bài mới chấp nhận/từ chối tên mình |
| `publishing_schedules` | Lịch đăng: một revision chỉ một lịch còn hiệu lực (hai admin reserve đua nhau thì một người thắng), chặn giờ yên lặng của user đích, pilot ≤1 bài/user/ngày |
...
| `publishing-api` | Kho bài AI (R5): nhập batch all-or-nothing ≤20 bài/1MB, duyệt tạo revision bất biến, phân lịch và lịch tuần; user tự xem "Bài sắp đăng của tôi" và Chấp nhận/Từ chối. Server không nhận cookie/CSRF TechHub — extension là nơi thực thi (R6) | `ADMIN_TOKEN` cho action admin; máy user dùng device token đã enrollment ở engagement-api |
...
SELECT p.status, count(*) FROM publishing_schedules p GROUP BY p.status;
SELECT a.decision, count(*) FROM content_revision_approvals a GROUP BY a.decision;
```
